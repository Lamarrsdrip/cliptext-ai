...
export function TranscriptPanel(props: TranscriptPanelProps) {
  ...
  function downloadSrt() {
    ...
    downloadBlob(srt, "text/srt;charset=utf-8", `${sanitizeFilename(recordingTitle ?? "transcript")}.srt`);
  }

  function downloadTxt() {
    const txt = displaySegments.map((s) => s.text).join(" ");
    downloadBlob(txt, "text/plain;charset=utf-8", `${sanitizeFilename(recordingTitle ?? "transcript")}.txt`);
  }

  function downloadMd() {
    const title = recordingTitle ?? "Transcript";
    const lines = displaySegments.map((s) => `**[${msToClock(s.startMs)}]** ${s.text}`);
    const md = `# ${title}\n\n${lines.join("\n\n")}`;
    downloadBlob(md, "text/markdown;charset=utf-8", `${sanitizeFilename(recordingTitle ?? "transcript")}.md`);
  }

  function copyClaudePrompt() {
    const title = recordingTitle ?? "Recording";
    const segments = displaySegments.map((s) => `[${msToClock(s.startMs)}] ${s.text}`).join("\n");
    const prompt = `Here is a timestamped transcript from a video/screen recording titled "${title}":\n\n<transcript>\n${segments}\n</transcript>\n\nPlease help me understand this recording.`;
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-2 p-3 border-b border-border">
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button variant="ghost" size="icon" onClick={downloadTxt}>
              <IconFileText className="h-4 w-4" />
            </Button>
          </TooltipTrigger>
          <TooltipContent>Save as .txt</TooltipContent>
        </Tooltip>
        <Tooltip>
          <TooltipTrigger asChild>
          ...
          <TooltipContent>Save as .md (with timestamps)</TooltipContent>
        </Tooltip>
        <Tooltip>
          <TooltipTrigger asChild>
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button variant="ghost" size="icon" onClick={copyClaudePrompt}>
              {copiedPrompt ? (
                <IconCheck className="h-4 w-4 text-green-600" />
              ) : (
                <IconRobot className="h-4 w-4 text-purple-500" />
              )}
            </Button>
          </TooltipTrigger>
          <TooltipContent>Copy Claude-ready prompt</TooltipContent>
        </Tooltip>
      </div>
